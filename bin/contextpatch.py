...
def context_patch(fs_file, dir_path) -> tuple:  # 接收两个字典对比
    ...
    for i in scan_dir(os.path.abspath(dir_path)):
        ...
        if fs_file.get(i):
            ...
            print(f"添加 [{i} {permission}], 可能会存在错误")
            add_new += 1
...
def main(dir_path, fs_config) -> None:
    ...
    print('ContextPatcher: 添加 %d' % add_new + " entries")


def Usage():
    print("Usage:")
    print("%s <folder> <fs_config>" % (sys.argv[0]))
    print("    一个自动修补file_context的脚本")
